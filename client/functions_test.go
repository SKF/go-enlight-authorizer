...
func clientFor(t *testing.T, server *authMock.AuthorizeServer) authorize.AuthorizeClient {
	...
	require.NoError(t, client.Dial(ctx, host, port, grpc.WithTransportCredentials(insecure.NewCredentials())))
...
func Test_IsAuthorizedBulkWithResources(t *testing.T) {
	...
	res, oks, err := client.IsAuthorizedBulk(ctx, "testUser", "testAction", []*common.Origin{
		{
			Id:       "0",
			Type:     "node",
			Provider: "1",
